...
kpi =spark.sql("select Sales_fact.market_id as Market_id,Sales_fact.product_group_id as product_group, Sum (Sales_fact.volume_units) as Total_volume_units,sum(Sales_fact.dollars) as total_revenue  from Fact_Table Sales_fact left outer join   prdgrp prdgrp on Sales_fact.market_id=prdgrp.market_id and Sales_fact.product_group_id =prdgrp.product_group_id group by Sales_fact.market_id,Sales_fact.product_group_id")
kpi.coalesce(1).write.mode("overwrite").format("csv").save("s3n://aws-a0049-use1-00-d-s3b-bpod-bdp-data01/onboarding/training/bv20370/results_from_airflow/")
spark.stop()
